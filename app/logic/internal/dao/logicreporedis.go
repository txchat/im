...
func (repo *LogicRepositoryRedis) GetMember(c context.Context, key string) (appId string, uid string, err error) {
	conn := repo.redis.Get()
	defer conn.Close()
...
func (repo *LogicRepositoryRedis) KeysByUIDs(c context.Context, appId string, uids []string) (ress map[string]string, onlyUID []string, err error) {
	conn := repo.redis.Get()
	defer conn.Close()
...
func (repo *LogicRepositoryRedis) AddMapping(c context.Context, uid string, appId string, key string, server string) (err error) {
	conn := repo.redis.Get()
	defer conn.Close()
	var n = 4
	if uid != "" {
		if err = conn.Send("HSET", keyUIDServer(appId, uid), key, server); err != nil {
			log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(HSET %s,%s,%s,%s) error", appId, uid, server, key))
			return
		}
		if err = conn.Send("EXPIRE", keyUIDServer(appId, uid), repo.redisExpire); err != nil {
			log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(EXPIRE %s,%s,%s,%s)", appId, uid, key, server))
			return
		}
		n += 2
	}
	if err = conn.Send("SET", keyKeyServer(key), server); err != nil {
		log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(HSET %s,%s,%s) error", uid, server, key))
		return
	}
	if err = conn.Send("EXPIRE", keyKeyServer(key), repo.redisExpire); err != nil {
		log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(EXPIRE %s,%s,%s) error", uid, key, server))
		return
	}
	user := fmt.Sprintf(UIDFmt, appId, uid)
	if err = conn.Send("SET", keyKeyUser(key), user); err != nil {
		log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(HSET %s,%s,%s) error", uid, appId, key))
		return
	}
	if err = conn.Send("EXPIRE", keyKeyUser(key), repo.redisExpire); err != nil {
		log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(EXPIRE %s,%s,%s) error", uid, appId, key))
		return
	}
	if err = conn.Flush(); err != nil {
...
func (repo *LogicRepositoryRedis) ExpireMapping(c context.Context, uid string, appId string, key string) (has bool, err error) {
	conn := repo.redis.Get()
	defer conn.Close()
	var n = 2
	if uid != "" {
		if err = conn.Send("EXPIRE", keyUIDServer(appId, uid), repo.redisExpire); err != nil {
			log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(EXPIRE %s) error", keyUIDServer(appId, uid)))
			return
		}
		n++
	}
...
func (repo *LogicRepositoryRedis) DelMapping(c context.Context, uid string, appId string, key string) (has bool, err error) {
	conn := repo.redis.Get()
	defer conn.Close()
	var n = 2
	if uid != "" {
		if err = conn.Send("HDEL", keyUIDServer(appId, uid), key); err != nil {
			log.Error().Str("key", key).Err(err).Msg(fmt.Sprintf("conn.Send(HDEL %s) error", keyUIDServer(appId, uid)))
			return
		}
		n++
	}
...
// KeysByUIDs key=server value=gid list
