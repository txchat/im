...
func (c *Conn) readFrame() (fin bool, op int, payload []byte, err error) {
	...
	case 126:
		// 16 bits
		if p, err = pop(c.r, 2); err != nil {
			return
		}
		payloadLen = int64(binary.BigEndian.Uint16(p))
	case 127:
		// 64 bits
		if p, err = pop(c.r, 8); err != nil {
			return
		}
	...
	// read payload
	if payloadLen > 0 {
		if payload, err = pop(c.r, int(payloadLen)); err != nil {
			return
		}
		if mask {
